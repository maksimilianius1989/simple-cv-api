...
import { GenerateCvPreviewCommand } from './generate-cv-preview. command';
...
import {
  CvPreviewFailedEvent,
  CvPreviewGeneratedEvent,
} from '@preview/application/events/cv.events';

@CommandHandler(GenerateCvPreviewCommand)
export class GenerateCvPreviewHandler implements ICommandHandler<GenerateCvPreviewCommand> {
  constructor(
  ...
  async execute(command: GenerateCvPreviewCommand): Promise<any> {
    ...
    try {
      ...
      this.eventBus.publish(new CvPreviewGeneratedEvent(cvId));
    } catch (error) {
      const reason =
        error instanceof Error ? error.message : 'Cv preview generation failed';
      this.eventBus.publish(new CvPreviewFailedEvent(cvId, reason));
    }
  }
}
